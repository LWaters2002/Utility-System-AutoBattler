...
[CreateAssetMenu(fileName = "DefensiveMove_Task", menuName = "ScriptObjects/Task/DefensiveMove", order = 0)]
public class DefensiveMove_Task : ATask
{
    public override void Entry()
    {
        ...
        if (heatTiles.Count != 0)
        {
            ...
            {
                if (tile.defence > bestTile.defence) bestTile = tile;
                if (tile.defence == bestTile.defence && Random.Range(0, 2) == 1) bestTile = tile;
            }

            if (bestTile.defence != 0)
            {
        ...
        // Otherwise move towards the nearest ally.
